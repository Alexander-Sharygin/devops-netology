...
```text
Опечатка в значении platform_id, также кол-во ядер 1 не доступно ни на какой платформе, везде минимум 2, исправил на standard_v1 и кол-во ядер 2.**
```
5. Ответьте, как в процессе обучения могут пригодиться параметры```preemptible = true``` и ```core_fraction=5``` в параметрах ВМ? Ответ в документации Yandex cloud.
```text
Параметры эти пригодятся для снижения стоимости ВМ, для обучения таких настроек ВМ достаточно, при самой низкой стоимости ресурса.
preemptible = true
...
ВМ будет иметь доступ к физическим ядрам как минимум 5% времени — 200 миллисекунд в течение каждой секунды. Тактовая частота процессора в это время не ограничивается и соответствует выбранной платформе.
```
...
- ответы на вопросы.  
```text
Ответы на вопросы выше, под вопросами.
```
...
**vms_platform.tf - добавил еще vm-all для одинаковых переменных для обеих вм.**
```terraform
###Переменные по заданию 2
variable "image_family" {
  type        = string
